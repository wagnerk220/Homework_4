...
def test(model, dataloader, device, criterion=None):
    ...
    Returns:
        tuple: (targets, outputs, probabilities, accuracy)
            ...
            - loss (float or None): Average loss when a criterion is provided.
    """
    model.eval()
    with torch.no_grad():
        ...
        loss = total_loss / float(seen_samples or len_dataset) if criterion is not None else None

    return targets, outputs, probabilities, accuracy, loss
